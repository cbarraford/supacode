...
struct RepositoriesFeatureTests {
  ...
  @Test func worktreeInfoEventRepositoryPullRequestRefreshUsesCLIProviderForGitLabRemote() async {
    ...
    state.githubIntegrationAvailability = .available
    state.reconcileSidebarForTesting()
    let mergeRequest = makePullRequest(
      providerID: .gitLab,
    ...
    let store = TestStore(initialState: state) {
      ...
      $0.githubCLI.batchPullRequests = { _, _, _, _ in
        Issue.record("GitLab refresh should not call GitHub batchPullRequests")
        return [:]
      }
      $0.cliForgeProvider.batchPullRequests = { remote, branches, _ in
        #expect(remote.providerID == .gitLab)
        #expect(branches == [featureWorktree.name])
    ...
    await store.send(
      .worktreeInfoEvent(
        .repositoryPullRequestRefresh(
          repositoryRootURL: URL(fileURLWithPath: repoRoot),
          worktreeIDs: [featureWorktree.id],
        )
      )
    ) {
      $0.inFlightPullRequestRefreshRepositoryIDs = [repository.id]
    ...
    await store.receive(\.repositoryPullRequestsLoaded)
    await store.receive(\.sidebarItems[id: featureWorktree.id].pullRequestChanged) {
      $0.sidebarItems[id: featureWorktree.id]?.pullRequest = mergeRequest
      $0.sidebarItems[id: featureWorktree.id]?.pullRequestBranchAtQueryTime = nil
    }
    await store.receive(\.repositoryPullRequestRefreshCompleted) {
      $0.inFlightPullRequestRefreshRepositoryIDs = []
