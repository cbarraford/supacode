...
struct CommandPaletteFeatureTests {
  ...
  @Test func commandPaletteShowsOnlyOpenActionForGitLabReadOnlyProvider() {
    let rootPath = "/tmp/repo"
    let worktree = makeWorktree(id: "\(rootPath)/wt-gitlab", name: "feature", repoRoot: rootPath)
    let repository = makeRepository(rootPath: rootPath, name: "Repo", worktrees: [worktree])
    var state = RepositoriesFeature.State(reconciledRepositories: [repository])
    state.selection = .worktree(worktree.id)
    state.setWorktreeInfoForTesting(
      ...
      pullRequest: makePullRequest(
        ...
        checks: [
          ForgeStatusCheck(
            detailsUrl: "https://gitlab.com/platform/mobile/ios/app-shell/-/jobs/1",
            status: "COMPLETED",
            conclusion: "FAILURE",
          )
        ],
    ...
    let pullRequestItems = CommandPaletteFeature.commandPaletteItems(from: state)
      .filter { item in
        switch item.kind {
    ...
    #expect(pullRequestItems.map(\.title) == ["Open MR"])
  }
